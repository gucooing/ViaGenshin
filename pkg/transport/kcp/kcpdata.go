...
func loopReadFromUDP(conn *net.UDPConn, onControlData onControlDataFunc, onSegmentData onSegmentDataFunc) {
	...
	for {
		...
		if err != nil {
			logger.Error("exit udp recv loop, error: %v, addr: %v", err, addr.String())
			return
		}
		...
		if n == 20 {
			logger.Debug("received control data %d from %s: %s", n, addr, hex.EncodeToString(b[:n]))
		...
		if err != nil {
			logger.Error("receive error: %v from %s", err, addr)
		}
	}
}
...
func writeControlDataToUDP(conn *net.UDPConn, data *controlData, addr *net.UDPAddr) error {
	...
	logger.Debug("sending control data %d to %s: %s", len(data), addr, hex.EncodeToString(data[:]))
	return err
}
